...
import StartUpIcon from '../Images/PrototypeIcon.png';
...
export default function MyStartup() {
  ...
  let getData = () => {
    function onSuccess(response) {
      ...
      if(response.val()) {
        const cards = Object.entries(response.val()).map(startupObj => {
          const startupData = startupObj[1];
          if(startupData.public) {
          return <Card key={startupObj[0]} className={classes.root} onClick={() => openMoreInfoDialog(startupData)}>
          <CardActionArea>
            <CardContent>
